...
})({"723kW":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
const handleInput = (e)=>{
    ...
    if (e.target.value === '0') {
        document.querySelector(`.input-error.${e.target.id}`).style.display = 'block';
        e.target.style.outlineColor = 'hsl(13, 70%, 61%)';
    } else {
        document.querySelector(`.input-error.${e.target.id}`).style.display = 'none';
        e.target.style.outlineColor = 'hsl(172, 67%, 45%)';
    }
};
const displayResults = ()=>{
    if (amount.value && people.value && people.value !== '0') {
        tipAmountPerPerson.textContent = (parseFloat(amount.value) * tipPercentage / 100 / parseInt(people.value)).toFixed(2);
        totalPerPerson.textContent = (parseFloat(amount.value) * (1 + tipPercentage / 100) / parseInt(people.value)).toFixed(2);
    }
};
...
customTip.addEventListener('input', (e)=>{
    ...
    } else e.target.style.background = 'hsl(189, 41%, 97%)';
});
...
},{}]},["723kW","8lqZg"], "8lqZg", "parcelRequiredaa2")
